...
describe("SCHOOL| UPDATE | PUT /api/school/{id}", () => {
  let server;
  let superUser;
  let normalUser;
  ...
  it("responds with 400 when passed incorrect payload", async () => {
    const res = await server.inject({
      method: "put",
      url: "/api/school/" + school._id,
      headers: { authorization: superAccessToken },
  ...
  it("responds with 404 when unkown id is passed", async () => {
    const res = await server.inject({
      method: "put",
